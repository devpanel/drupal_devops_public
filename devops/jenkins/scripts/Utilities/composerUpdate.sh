...
./runInOneInstanceOnCluster.sh $REGION $CLUSTER_NAME "`aws ecr get-login --no-include-email --region $REGION` && docker pull $ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/drupal:$VERSION_IMAGE_INSTALL && docker run -v /efs/$CONTAINER_DB_NAME:/var/www/html/. $ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/drupal:$VERSION_IMAGE_INSTALL /composer_update.sh"
